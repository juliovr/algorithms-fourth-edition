...
public class Exercise8_Diameter {
    ...
    public static void main(String[] args) {
        ...
        double diameter = findDiameter(edgeWeightedDigraph);
        System.out.println("Diameter: " + diameter + " Expected: 1.86");
    }

    private static double findDiameter(EdgeWeightedDigraph graph) {
        double diameter = Double.NEGATIVE_INFINITY;

        for (int v = 0; v < graph.V(); ++v) {
            DijkstraSP sp = new DijkstraSP(graph, v);

            for (int w = 0; w < graph.V(); ++w) {
                if (diameter < sp.distTo(w)) {
